...
class SchedulerRequestReceiver:
    ...
    def _broadcast_within_attention_dp_group(
        ...
        """Broadcast requests from the (CP0, attention-TP0) DP leader.

        Attention ranks use a two-dimensional (CP, attention-TP) layout.  Only
        the DP leader receives requests initially, so first seed the CP0
        attention-TP row, then let that row seed every CP column.  Starting an
        attention-TP broadcast in the other CP rows would make their source
        ranks try to broadcast ``None``.
        """
