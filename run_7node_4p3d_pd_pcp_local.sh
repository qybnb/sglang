...
export DECODE_MAX_RUNNING_REQUESTS="${DECODE_MAX_RUNNING_REQUESTS:-6}"
...
# dSparK block=7 produces an 8-token target-verify window.  Decode uses
# attention-TP48 with DeepEP, so target-verify graph rows must satisfy
# batch_size * 8 % 48 == 0.  The smallest valid request-batch bucket is 6.
export CUDA_GRAPH_BS="${CUDA_GRAPH_BS:-6}"
