...
@unittest.skipUnless(is_npu(), "Test requires Ascend NPU")
class TestKDATargetVerifyNPU(unittest.TestCase):
    def test_speculative_cache_uses_key_value_layout(self):
        """The target-verify kernel must consume speculative [K, V] states.

        Kimi-K3 has K == V, which lets an accidental transpose pass every
        shape check.  Deliberately use different dimensions here so the test
        guards both the stride interpretation and the recurrent equation.
        """
        ...
        # Model-side target verify passes an already activated log-decay and
        # sigmoid beta, indicated by their leading singleton dimensions.
        a = -torch.rand(1, tokens, h_k, key_dim, device=device)
        b = torch.rand(1, tokens, h_v, device=device)
        initial = torch.randn(
            pool, h_v, key_dim, value_dim, device=device, dtype=dtype
        )
        ...
        for step in range(steps):
            ...
            value = v_ref[:, step] - torch.matmul(
                k_step.unsqueeze(2), state
            ).squeeze(2)
